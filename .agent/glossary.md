# Glossary - Nishaniyaan Terminology

## Core Terms

| Term | Meaning |
|------|---------|
| **Nishaniyaan** | "The Signs" - Project name |
| **Dajjal** | Al-Masih ad-Dajjal (The False Messiah) |
| **Al-Masih** | The Messiah |
| **Qiyamah** | The Day of Judgment (*Ashrat as-Sa'ah*) |
| **Imam Mahdi** | Righteous leader from the lineage of Prophet Muhammad ﷺ |
| **Isa (AS)** | Prophet Jesus, Son of Mary (Peace Be Upon Him) |

## Verification Badges

| Badge | Meaning |
|-------|---------|
| 🟢 Authentic | Qur'an - Explicit divine verses |
| 🟢 Sahih | Sahih Hadith - Canonical Bukhari & Muslim |
| 🟡 Hasan | Acceptable transmission |
| 🟠 Da'if | Weak chain |
| 🔴 Mawdu' | Fabricated, rejected |
| ⚪ Scholarly Opinion | Classical exegesis |
| 🔵 Historical Record | Documented events |
| 🟣 Observation | Contemporary trends |
| ⚫ Unverified | Quarantined rumors |

## Content Processing Terms

| Term | Meaning |
|------|---------|
| **Sanitization** | Removing identities, metadata, promotional content |
| **Translation** | Converting non-English content to English |
| **Verification** | Checking against Qur'an and Hadith sources |
| **Classification** | Categorizing content by type |
| **Distribution** | Assigning to correct chapter/slide |
| **Quarantine** | Placing unverified claims in separate section |
| **Integration** | Updating the website with processed content |

## Language Policy

- **All content must be in English**
- Any input in any language MUST be translated
- Translation must preserve meaning and context
- Arabic terms may be kept with transliteration
- No non-English content on the website

## Phases

| Phase | Period | Key Event |
|-------|--------|-----------|
| 1 | Before Dajjal | Minor Signs Era |
| 2 | Just Before | Near Signs & Precursors |
| 3 | Imam Mahdi | Righteous Emergence |
| 4 | 439 Days | Dajjal's Rule |
| 5 | Descent | Isa (AS) |
| 6 | After | Golden Age & Victory |
| 7 | To Qiyamah | Final Horn Blasts |

## 439 Days Calculation

| Period | Duration | Equivalent |
|--------|----------|------------|
| Day 1 | 1 Year | 360 days |
| Day 2 | 1 Month | 30 days |
| Day 3 | 1 Week | 7 days |
| Days 4-40 | 37 days | 37 days |
| **Total** | - | **434-439 days** |
